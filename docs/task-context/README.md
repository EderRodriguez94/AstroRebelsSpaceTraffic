...
Without `ExactFileList`, the extractor reads the repository's two source-of-truth documents. With it, provide up to six literal file paths; wildcards are rejected. Missing or duplicate IDs, missing files, and output larger than `MaxChars` fail with a non-zero exit code and an explicit message. File integrity and history are managed through Git commits.
